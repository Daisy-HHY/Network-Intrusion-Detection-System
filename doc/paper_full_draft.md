...
Denning 较早提出了入侵检测模型，认为系统行为可以通过审计记录、统计特征和异常模式进行刻画，并可据此发现偏离正常行为的入侵活动[1]。在网络入侵检测场景中，检测对象通常表现为网络连接或流量记录，特征包括协议类型、连接状态、字节数、包间隔、连接计数等。由于这些特征具有较强的结构化表格数据特征，机器学习方法能够在人工规则之外学习正常流量与攻击流量之间的统计差异。国内关于工业互联网和网络空间安全的综述也指出，机器学习与深度学习已成为入侵检测中处理复杂流量特征和异常行为识别的重要技术路线[11-12]。
...
在表格型安全数据上，树模型和集成学习方法被广泛采用。随机森林通过构建多棵决策树并进行集成，降低单棵树的方差[4]；梯度提升机通过逐步拟合残差提升模型表达能力[5]；XGBoost 在梯度提升框架中引入正则化、列采样、并行优化等机制，提升了树提升模型的效率和泛化能力[6]；LightGBM 进一步通过基于直方图的算法和叶子优先生长策略提高训练效率[7]；CatBoost 则针对类别特征处理和有序提升提出改进[8]。国内已有研究将随机森林特征选择与 XGBoost 分类相结合用于网络入侵检测[13]，也有研究融合随机森林与梯度提升树处理入侵检测中的数据不平衡多分类问题[14]，说明树模型和集成学习在该任务中具有较高研究热度。

近年来，深度学习也被用于表格数据建模。Gorishniy 等对表格深度学习模型进行了系统比较，并指出 ResNet-like 与 FT-Transformer 等结构可以作为较强的深度表格模型基线，但深度模型与梯度提升树之间并不存在普遍意义上的绝对优势[9]。国内基于深度学习的网络入侵检测综述同样表明，CNN、RNN、自编码器、GAN、MLP 等模型已被广泛用于入侵检测研究，但模型效果仍受数据集、特征表示、类别不均衡和评价指标影响[18-19]。这与本文实验现象基本一致：在当前 UNSW-NB15 二分类任务中，普通 MLP 未超过 XGBoost；表格 ResNet-like 模型相较普通 MLP 有改进，但在同口径子集实验中仍略低于 XGBoost，且训练耗时更长。
...
本文使用 UNSW-NB15 数据集。根据 UNSW 官方说明，该数据集由澳大利亚网络安全中心的 Cyber Range Lab 生成，包含正常流量和多种现代攻击类型，并提供训练集和测试集划分[2]。已有国内研究也直接基于 UNSW-NB15 数据集开展入侵检测建模，例如结合 DBSCAN、GAN 与 XGBoost 处理稀有攻击类样本并进行分类[15]，说明该数据集在机器学习入侵检测研究中具有较高应用价值。本文项目中的原始数据文件为：
...
Decision Tree 的优点是路径规则直观，每次预测都可以追溯到一系列特征划分；缺点是单棵树容易过拟合训练数据。Random Forest 通过训练多棵树并集成预测结果缓解单棵树不稳定问题。本文二分类中 Random Forest 的 F1 为 0.8936，高于单棵 Decision Tree 的 0.8853，符合集成方法通常比单模型更稳定的特点。类似地，已有入侵检测研究也通过随机森林、梯度提升树等集成模型提升多分类检测效果[14]。但 Random Forest 仍低于 XGBoost，说明在当前任务中，逐步提升式集成比简单 bagging 集成更适合。

XGBoost、LightGBM、CatBoost 和 HistGradientBoosting 均属于梯度提升思想下的重要模型或实现。它们的共同特点是通过多轮弱学习器叠加逐步改善预测结果；差异主要体现在工程优化、树生长策略、类别特征处理和正则化设计上。国内已有研究使用人工蜂群算法进行特征选择后结合 XGBoost 降低入侵检测虚警率[16]，也有研究在 UNSW-NB15 上使用 XGBoost 作为核心分类器[15]，这与本文将 XGBoost 作为重点比较模型的设计相呼应。本文扩展实验中 LightGBM、XGBoost reference、HistGradientBoosting 和 CatBoost 排名前四，F1 均达到 0.8947 以上，说明梯度提升类模型整体适合当前二分类入侵检测任务。
...
表格数据通常由数值特征、类别特征和统计特征组成，与图像、语音、文本等数据不同，其特征之间缺少天然的空间或序列结构。因此，在许多表格任务中，梯度提升树仍是强有力的基线。Gorishniy 等指出，表格深度学习领域存在模型比较协议不统一的问题，并提出 ResNet-like 和 FT-Transformer 等结构作为更可靠的深度表格模型基线[9]。从网络入侵检测领域看，相关中文综述也总结了深度学习模型在入侵检测数据集、特征学习和检测性能方面的应用现状，并指出深度模型仍面临训练效率、解释性和场景适配等问题[18-19]。
...
普通 MLP 将所有输入特征经过编码和标准化后视为一个稠密向量，再通过全连接层学习特征组合。其优点是结构通用，能够拟合非线性关系；缺点是对表格数据中的类别特征、稀疏独热特征和特征尺度较敏感，且通常需要较多调参。相关研究在入侵检测任务中也将 MLP 与逻辑回归、决策树、随机森林等传统模型进行对比，说明神经网络常作为网络流量分类的重要基线模型[17]。本文 MLP 改进实验尝试了更长训练和更宽网络，但 F1 均低于基础 MLP，说明简单增加模型容量并不一定带来更好效果。
...
系统设计遵循两个原则。第一，二分类和多分类共享相同的数据读取与预处理逻辑，避免因不同模型输入处理不一致导致比较偏差。第二，所有模型均使用官方训练集训练、官方测试集评估，不在测试集上进行训练或重采样，从而保证测试结果能够反映模型在固定测试划分上的泛化表现。已有基于特征工程和集成学习的网络入侵检测研究表明，数据清洗、特征处理、类别不平衡处理与模型选择会共同影响检测效果[20]，因此本文将预处理流程固定后再比较不同模型。
...
从模型族角度看，树模型整体优于线性模型。Decision Tree 的 F1 为 0.8853，高于 Logistic Regression 的 0.8492；Random Forest 的 F1 进一步提高到 0.8936，说明集成多棵树能够改善单棵树的稳定性和泛化效果；XGBoost 在此基础上取得 0.8968 的 F1，说明梯度提升树在当前二分类任务中具有更强拟合能力。这与国内基于随机森林、XGBoost 以及多模型对比的入侵检测研究结论相吻合[13-14,17]。
...
多分类 Macro F1 明显低于二分类 F1，主要原因来自两个方面。第一，多分类任务需要区分具体攻击类别，分类边界更复杂。第二，数据集中类别分布不均衡，Worms、Shellcode、Backdoor 等少数类训练样本较少，模型难以充分学习其稳定特征。已有融合随机森林和梯度提升树的入侵检测研究也将数据不平衡视为影响多分类检测效果的重要因素[14]。当前项目仅保存了多分类整体 macro 指标，未保存每个攻击类别的详细分类报告，因此本文不能进一步断言某个具体攻击类别识别效果最好或最差。
...
结果表明，延长训练轮数或扩大隐藏层宽度并未提升 F1。`mlp_tuned_longer` 和 `mlp_tuned_wider` 的 Recall 略高或接近基础 MLP，但 Precision 明显下降，导致 F1 低于基础 MLP。这说明在当前设置下，MLP 未超过 XGBoost 并不只是由于训练轮数不足或网络规模不足造成的，也可能与普通 MLP 对表格特征交互的建模能力有关。相关文献虽然表明 MLP、CNN 等深度学习模型可用于入侵检测，但也强调其效果依赖具体数据特征、模型结构和实验设置[17-19]。
...
扩展实验显示，LightGBM 取得最高 F1，为 0.8975，略高于 `xgboost_reference` 的 0.8969；LightGBM 的 ROC-AUC 为 0.9855，也略高于 `xgboost_reference` 的 0.9842。两者 F1 差值约为 0.00058，提升幅度很小。国内基于 LightGBM 的高效入侵检测系统研究也说明，LightGBM 可作为网络入侵检测中的有效梯度提升模型[21]。因此，本文仅将 LightGBM 表述为扩展实验中表现最好的补充对照模型，而不将其描述为显著优于 XGBoost。

HistGradientBoosting 和 CatBoost 也取得较高性能，F1 分别为 0.8956 和 0.8947，但均低于 XGBoost reference。相关 CatBoost 改进入侵检测方法也表明，针对类别不平衡和特征处理改进的梯度提升模型在入侵检测中具有应用价值[22]。ExtraTrees、GradientBoosting、AdaBoost tree、Linear SVM SGD 和 GaussianNB 表现逐步下降。其中 GaussianNB 虽然 Precision 接近 1，但 Recall 仅为 0.1941，说明其只识别出少量攻击样本，不适合作为当前系统主模型。
...
后续工作可从三个方向展开。第一，对 XGBoost 和 LightGBM 进行更系统的超参数搜索，并结合阈值调整分析误报与漏报之间的权衡。第二，补充多分类每类 precision、recall、F1 和混淆矩阵，重点分析 Worms、Shellcode、Backdoor 等少数类攻击的识别问题。第三，在保证特征可实时获取的前提下，探索在线流量特征提取、模型推理接口和告警输出流程，使离线实验系统逐步扩展为可验证的实时检测原型。除此之外，面向网络空间防御的对抗机器学习研究提示，后续还可进一步关注入侵检测模型在对抗样本、数据漂移和规避攻击条件下的鲁棒性[23]。
...
总体而言，本文基于真实项目实验结果得出的核心结论是：在当前 UNSW-NB15 离线实验设置下，梯度提升类树模型是最有效的模型族；XGBoost 是默认模型集合中的主模型，LightGBM 是扩展实验中表现最好的补充模型；深度学习方法具有研究价值，但在当前实验配置下没有超过树提升模型。该结论与国内外关于集成学习、XGBoost、LightGBM 和深度学习入侵检测的相关研究形成相互印证[13-21]，但仍受数据集、预处理方式、参数设置和评价指标限制，后续需要通过更系统的实验进一步验证和完善。
...
[11] 刘奇旭, 陈艳辉, 尼杰硕, 等. 基于机器学习的工业互联网入侵检测综述[J]. 计算机研究与发展, 2022, 59(5): 994-1014.
...
[14] 周杰英, 贺鹏飞, 邱荣发, 等. 融合随机森林和梯度提升树的入侵检测研究[J]. 软件学报, 2021, 32(10): 3254-3265.

[15] 张启坤, 黄道颖, 刘江豪, 等. 基于DBSCAN_GAN_XGBoost的网络入侵检测方法[J]. 郑州大学学报(工学版), 2022, 43(1): 30-36.

[16] 徐伟, 冷静. 基于人工蜂群算法和XGBoost的网络入侵检测方法研究[J]. 计算机应用与软件, 2021, 38(3): 314-318, 333.

[17] 任家东, 张亚飞, 张炳, 等. 基于特征选择的工业互联网入侵检测分类方法[J]. 计算机研究与发展, 2022, 59(5): 1148-1159.

[18] 肖建平, 龙春, 赵静, 等. 基于深度学习的网络入侵检测研究综述[J]. 数据与计算发展前沿, 2021, 3(3): 59-74.

[19] 苏书宾, 肖利民, 李书攀, 等. 基于深度学习的网络入侵检测研究综述[J]. 信息安全学报, 2026, 11(1): 79-101.

[20] 基于特征工程和集成学习的网络入侵检测方法[EB/OL]. 中国科技论文在线, 2024.

[21] 基于LightGBM算法的高效入侵检测系统[J]. 计算机应用与软件, 2025, 42(11): 待确认.

[22] 一种基于改进CatBoost算法的网络入侵检测方法: CN115567292A[P]. 2022.

[23] 面向网络空间防御的对抗机器学习研究综述[J]. 自动化学报, 待确认.
